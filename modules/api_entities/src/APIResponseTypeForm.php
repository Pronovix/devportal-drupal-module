...
class APIResponseTypeForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => '\Drupal\devportal_api_entities\Entity\APIResponseType::load',
      ],
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label API Response type.', [
          '%label' => $api_response_type->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label API Response type.', [
          '%label' => $api_response_type->label(),
        ]));
    }
