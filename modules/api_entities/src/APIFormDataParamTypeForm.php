...
class APIFormDataParamTypeForm extends EntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label API HTTP Method Form Data Parameter type.', [
          '%label' => $api_form_data_param_type->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label API HTTP Method Form Data Parameter type.', [
          '%label' => $api_form_data_param_type->label(),
        ]));
    }
