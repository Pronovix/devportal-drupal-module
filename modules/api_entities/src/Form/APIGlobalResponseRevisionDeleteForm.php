...
class APIGlobalResponseRevisionDeleteForm extends ConfirmFormBase {
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_type_manager->getStorage('api_global_response'),
      $connection,
      $date_formatter
    );
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->messenger()->addMessage($this->t('Revision from %revision-date of API Global Response %title has been deleted.', [
      '%revision-date' => $this->dateFormatter->format($this->revision->getRevisionCreationTime()),
      '%title' => $this->revision->label(),
    ]));
