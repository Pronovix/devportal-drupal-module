...
class DeleteMultipleAPIDocs extends ConfirmFormBase {
   ...
   * The APIDoc storage.
   *
   * @var \Drupal\devportal_api_entities\APIDocStorageInterface
   */
  protected $storage;
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    /** @var \Drupal\devportal_api_entities\APIDocInterface[] $api_docs */
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    if ($form_state->getValue('confirm') && !empty($this->apiDocInfo)) {
      ...
      if ($total_count) {
        $this->messenger()->addMessage($this->formatPlural($total_count, 'Deleted 1 API Documentation.', 'Deleted @count API Documentations.'));
      }
